...
```{r}
library(dplyr)
alc %>% group_by(sex, high_use) %>% summarise(count = n(), mean_grade=mean(G3), mean_absences=mean(absences), mean_failures=mean(failures), mean_health=mean(health))
...
With model 2, AIC is lower (which is good!) and all the remaining variables remain statistically significant. Then we should check for multicollinearity:
...
VIF is low, so no worries in that regard. Then, let's calculate the corresponding ODDS ratios and confidence intervals (95%):
