...
The deviance tells us how well our model fits the data, in terms of error. The coefficients are the _logarithmic odds_ that represent change in the response variable in respect to one unit change in the explanatory variable, keeping that the other variables stay put (conditionally on these). Logartimic link function is necessary here to provide equal scale to the response variable (odds from 0-100%, 0-1) and the explanatory variables (with also numerical values included). 
...
Odds can be tabulated using exp(Estimate) = propability/(1-probability) = ODDS. These are interpreted as odds ratios between one unit change vs. no change in the corresponding explanatory variable.

If the slopes of the fitted lines (model coefficients) are indifferent from zero, the variable is statistically significant. 

There is also AIC = Akaike information criterion, that tells us how economic the model is. That is, how good is the predictive ability when considering the number of variables included (too many might cause overfitting).

In this case, G3 seems unnecessary, so we could drop it.
...
Let's also calculate the ODDS ratios and confidence intervals (95%):
...
With model 2, AIC is lower (which is good!) and all the remaining variables remain statistically significant. 
...
However, the 95% CI crosses the value "1" in absences and sex. That means that based on this data, the true OR may be 1, which is no difference (per one unit change). 
