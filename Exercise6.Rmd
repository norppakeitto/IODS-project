...
# Print out summaries of the variables. HUOM. N = 40, mutta subjects on vain 1-20 joko treatment 1 tai treatment 2
...
# Convert to long form; ei muuteta muuttujia treatment ja subject
...
BPRSL <- BPRSL %>%
  ...
  ungroup()
# mielestäni sama tulos kuin näin
BPRSL <- BPRSL %>%
  group_by(week) %>%
...
library(dplyr)
library(tidyr)
# Summary data with mean and standard error of bprs by treatment and week. (= mean bprs by group and by week; se bprs by group and by week)
BPRSS <- BPRSL %>%
...
BPRSL8S1_outliersout <- BPRSL8S %>% filter(mean < 70)

library(ggplot2)
ggplot(BPRSL8S1_outliersout, aes(x = treatment, y = mean)) +
  geom_boxplot() +
  stat_summary(fun = "mean", geom = "point", shape=23, size=4, fill = "white") +
  scale_y_continuous(name = "mean(bprs), weeks 1-8")
